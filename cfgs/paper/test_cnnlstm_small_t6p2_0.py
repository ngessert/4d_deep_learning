...
def init(mdlParams_):            
    ...
    mdlParams['num_GRU_layers'] = 2
    # Number of FMs in GRU layers
    mdlParams['num_hidden'] = 128
    # Determines whether mean/var in batch norm are tracked per time step or not
    mdlParams['keep_time_dim'] = True
    # Pool features?
    mdlParams['use_pooled_features'] = True

    ### 4D CNN ResNetA Parameters ###
    # Parameters for the initial convolution
    mdlParams['strides_init'] = [1,2,2,2]
    mdlParams['num_filters_init'] = 16
    mdlParams['kernel_init'] = [1,3,3,3]
    mdlParams['factorize'] = False
    # Resnet architecture definition
    mdlParams['ResNet_Size'] = [1,2,2]
    # Strides
    mdlParams['ResNet_Stride'] = [[1,2,2,2],[1,2,2,2],[1,2,2,2]]
    # Kernels
    mdlParams['ResNet_Kernels'] = [[1,3,3,3],[1,3,3,3],[1,3,3,3]]
    # Feature map size
    mdlParams['ResNet_FM'] = [16,32,64]
    # Use LSTM instead of GRU (revision 1)
    mdlParams['use_lstm'] = True    
    ...
    for i in range(len(mdlParams['valIndCV'])):
        ...
        print("Intersect val",np.intersect1d(mdlParams['trainIndCV'][i],mdlParams['valIndCV'][i]))
