...
def init(mdlParams_):
    ...
    ### Model Selection ###
    mdlParams['model_type'] = 'convGRUCNN'
    #mdlParams['batch_func_type'] = 'getBatchOpto'
    mdlParams['input_func_type'] = 'force' 
    mdlParams['h5datanames'] = ['vols','forces_t_mean','indices_0']
    # All sets to be considered
    mdlParams['dataset_names'] = ['Force4D_2__1','Force4D_2__2','Force4D_2__3','Force4D_2__4','Force4D_2__5','Force4D_2__6','Force4D_2__7','Force4D_2__8','Force4D_2__9','Force4D_2__10','Force4D_2__11','Force4D_2__12']
    # Subset for validation, if no mix
    mdlParams['dataset_names_val'] = [['Force4D_2__7']]
    mdlParams['dataset_names_test'] = ['Force4D_2__4','Force4D_2__8']
    mdlParams['numCV'] = 1
    ...
    mdlParams['target_shift'] = 1

    ### convGRU Parameters ###
    # Number of initial convgru layers
    mdlParams['num_convGRU_layers'] = 1
    # Kernel size
    mdlParams['convgru_kernel'] = [5,5,5]
    # Number of FMs in convGRU layers
    mdlParams['num_hidden'] = 12
    # Determines whether mean/var in batch norm are tracked per time step or not
    mdlParams['keep_time_dim'] = True

    ### 3D CNN ResNetA Parameters ###
    # Parameters for the initial convolution
    mdlParams['strides_init'] = [2,2,2]
    mdlParams['num_filters_init'] = 16
    mdlParams['kernel_init'] = [3,3,3]
    # Resnet architecture definition
    mdlParams['ResNet_Size'] = [1,2,2]
    # Strides
    mdlParams['ResNet_Stride'] = [[2,2,2],[2,2,2],[2,2,2]]
    # Feature map size
    ...
    # Preload?
    mdlParams['preload'] = mdlParams_['preload']
        ...
        with h5py.File(mdlParams['data_paths'][i]) as vols:
            ...
            if i == 0:
                if mdlParams['preload']:
                    ...
                    # Expand
                    if len(mdlParams['data_array'].shape) == 4:
                        mdlParams['data_array'] = np.expand_dims(mdlParams['data_array'],axis=4)
                ...
                for k in range(mdlParams['numCV']):
                    ...
                    for set in mdlParams['dataset_names_val'][k]:
                        if set in mdlParams['data_paths'][i]:
                            mdlParams['valIndCV'][k] = np.concatenate((mdlParams['valIndCV'][k],np.array(np.arange(mdlParams['timesteps'],mdlParams['labels_array'].shape[0]-mdlParams['target_shift']))))
                            found = True
                            break     
                    for set in mdlParams['dataset_names_test']:
                        if set in mdlParams['data_paths'][i]:
                            mdlParams['testInd'] = np.concatenate((mdlParams['testInd'],np.array(np.arange(mdlParams['timesteps'],mdlParams['labels_array'].shape[0]-mdlParams['target_shift']))))
                            found = True
                            break                               
                    if not found:
                        mdlParams['trainIndCV'][k] = np.concatenate((mdlParams['trainIndCV'][k],np.array(np.arange(mdlParams['timesteps'],mdlParams['labels_array'].shape[0]-mdlParams['target_shift']))))
                mdlParams['valid_indices'] = np.array(np.arange(mdlParams['timesteps'],mdlParams['labels_array'].shape[0]-mdlParams['target_shift']))
                mdlParams['setids'] = np.zeros([mdlParams['labels_array'].shape[0]])
                mdlParams['local_indices'] = np.array(np.arange(0,mdlParams['labels_array'].shape[0]))
            else:
                if mdlParams['preload']:
                    new_vols = np.transpose(vols[mdlParams['h5datanames'][0]].value)
                    # Expand
                    if len(new_vols.shape) == 4:
                        new_vols = np.expand_dims(new_vols,axis=4)                    
                ...
                for k in range(mdlParams['numCV']):
                    ...
                    for set in mdlParams['dataset_names_val'][k]:
                        if set in mdlParams['data_paths'][i]:
                            mdlParams['valIndCV'][k] = np.concatenate((mdlParams['valIndCV'][k],np.array(np.arange(mdlParams['valid_indices'][-1]+mdlParams['timesteps']+mdlParams['target_shift'],mdlParams['valid_indices'][-1]+new_tars.shape[0]+1-mdlParams['target_shift']))))
                            found = True
                            break 
                    for set in mdlParams['dataset_names_test']:
                        if set in mdlParams['data_paths'][i]:
                            mdlParams['testInd'] = np.concatenate((mdlParams['testInd'],np.array(np.arange(mdlParams['valid_indices'][-1]+mdlParams['timesteps']+mdlParams['target_shift'],mdlParams['valid_indices'][-1]+new_tars.shape[0]+1-mdlParams['target_shift']))))
                            found = True
                            break                                   
                    if not found:
                        mdlParams['trainIndCV'][k] = np.concatenate((mdlParams['trainIndCV'][k],np.array(np.arange(mdlParams['valid_indices'][-1]+mdlParams['timesteps']+mdlParams['target_shift'],mdlParams['valid_indices'][-1]+new_tars.shape[0]+1-mdlParams['target_shift']))))                    
                mdlParams['valid_indices'] = np.concatenate((mdlParams['valid_indices'],np.array(np.arange(mdlParams['valid_indices'][-1]+mdlParams['timesteps']+mdlParams['target_shift'],mdlParams['valid_indices'][-1]+new_tars.shape[0]+1-mdlParams['target_shift']))))
                mdlParams['setids'] = np.concatenate((mdlParams['setids'],i*np.ones([new_tars.shape[0]])))
                mdlParams['local_indices'] = np.concatenate((mdlParams['local_indices'],np.array(np.arange(0,new_tars.shape[0]))))
    # To int
    mdlParams['setids'] = mdlParams['setids'].astype(int)
    mdlParams['local_indices'] = mdlParams['local_indices'].astype(int)
    print("Setids shape",mdlParams['setids'].shape,"Local inds",mdlParams['local_indices'].shape)
    # Expand
    if mdlParams['preload']:
        if len(mdlParams['data_array'].shape) == 4:
            mdlParams['data_array'] = np.expand_dims(mdlParams['data_array'],axis=4)
        ...
        if mdlParams['normalize']:      
            for i in range(len(mdlParams['data_array'])):
                for j in range(mdlParams['data_array'].shape[-1]):
                    im_min = np.min(mdlParams['data_array'][i,:,:,:,j])
                    im_max = np.max(mdlParams['data_array'][i,:,:,:,j])
    ...
    mdlParams['labels_array'] = mdlParams['labels_array'].astype(np.float32)           
    # Ind properties
    print("Train")
